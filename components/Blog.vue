    ...
    <div class="flat">
      ...
      <div class="cards">
        ...
        >
          ...
          <div class="card-body">
            <div v-if="post._embedded && post._embedded['wp:featuredmedia']" class="card-thumb">
              <img
              	width="150"
              	height="150"
            ...
            <div class="card-excerpt" v-html="truncateExcerpt(post.excerpt.rendered)"></div>
          </div>

          <div class="card-footer" v-if="post._embedded && post._embedded['wp:term']">
             <span v-for="term in post._embedded['wp:term'][0]" :key="term.id" class="label">
                {{ term.name }}
              </span>
          </div>
        </a>
      ...
      <div class="blog-link">
        <p>
          <a class="btn" :href="blogURL" target="_blank" title="read more">Visit My Blog</a>
        </p>
      </div>
    </div>
    <Brackets name="blog" />
...
export default {
  name: "Blog",
  emits: ['loaded'], // Added to explicitly declare the emit
  data() {
    return {
      posts: [],
      blogURL: blogURL,
...
<style scoped lang="scss">
  ...
  .cards {
  	...
  	> a {
  		...
  		transition: border 0.35s ease-in, transform 0.35s ease-in;
