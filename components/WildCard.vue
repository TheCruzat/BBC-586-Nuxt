<template>
  ...
  >
      ...
      <button class="pc-close" @click.stop="closeDrawer" aria-label="Close details">
        <span />
        <span />
      </button>
    <hr />
    <p class="pc-link">
      <a :href="url" :title="title" target="_blank" @click.stop>{{ displayUrl }}</a>
    </p>
    ...
    <div class="pc-drawer">
      ...
      <p v-if="studio" class="pc-studio">
        for: <a :href="studio.link" :title="studio.name" target="_blank" @click.stop>{{ studio.name }}</a>
      </p>

      <div v-if="team" class="project-team">
        <p>team:</p>
        <ul>
          <li v-for="t in team" :key="t.name">
            <a :href="t.link" target="_blank" @click.stop>{{ t.name }}</a> : {{ t.role }}
          </li>
        </ul>
      </div>
    </div>
...
<style lang="scss" scoped>
  ...
  .project-card {
    ...
    cursor: pointer;
    position: relative;
    transition: background 0.35s ease-in, border-color 0.35s ease-in;
    ...
    .project-team {
      ...
      p { font-size: 0.9rem; margin-bottom: 0.25rem; }
    }
